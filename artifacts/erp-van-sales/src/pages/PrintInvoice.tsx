...
import { getGetInvoiceQueryOptions, getGetCompanySettingsQueryOptions } from "@workspace/api-client-react";
...
export default function PrintInvoice() {
  ...
  const validId = !isNaN(invoiceId) && invoiceId > 0 && !!user;
  const { data: invoice, isLoading } = useQuery({
    ...getGetInvoiceQueryOptions(validId ? invoiceId : 0),
    enabled: validId,
  });
  ...
  const totalUnits = invoice?.items.reduce((s, i) => s + i.quantity, 0) ?? 0;

  useEffect(() => {
    if (!invoice || settingsLoading) return;
    document.title = `فاتورة ${invoice.invoiceNumber} — ${storeName}`;
    const timer = setTimeout(() => window.print(), 500);
    return () => clearTimeout(timer);
  }, [invoice, settingsLoading, storeName]);
  ...
  if (authLoading || isLoading || settingsLoading) return <div className="print-loading"><p>جارٍ تحميل الفاتورة...</p></div>;
  if (!user) return <div className="print-loading"><p>يجب تسجيل الدخول أولاً</p></div>;
  if (!invoice) return <div className="print-loading"><p>لم يتم العثور على الفاتورة</p></div>;
  ...
  return (
    <div className="receipt" dir="rtl">
      ...
      <table className="meta-table">
        <tbody>
          <tr>
            ...
            <td className="value strong">{invoice.invoiceNumber}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td className="value">{invoice.clientName}</td>
          </tr>
          <tr>
            <td className="label">البائع</td>
            <td className="value">{invoice.truckName}</td>
          </tr>
          <tr>
            <td className="label">الدفع</td>
            <td className="value strong">{invoice.paymentType === "cash" ? "نقداً" : "آجل"}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div className="total-section">
        ...
        <span className="total-amount">{formatDZD(invoice.totalAmount)}</span>
      </div>
