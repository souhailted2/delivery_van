...
router.get("/clients", async (req, res) => {
  ...
  res.json(clients.map((c) => ({ ...c, balance: Number(c.balance), creditLimit: c.creditLimit != null ? Number(c.creditLimit) : null })));
});
...
router.get("/clients/:id", async (req, res) => {
  ...
  res.json({ ...client, balance: Number(client.balance), creditLimit: client.creditLimit != null ? Number(client.creditLimit) : null });
});
...
router.put("/clients/:id", async (req, res) => {
  ...
  const { name, phone, clientType, latitude, longitude, balance, branchId, creditLimit } = req.body;
  const validTypes = ["retail", "half_wholesale", "wholesale"];
  ...
  if (creditLimit !== undefined) updates.creditLimit = creditLimit !== null && creditLimit !== "" ? String(creditLimit) : null;
